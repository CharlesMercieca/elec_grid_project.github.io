...
###
fit_inp <- fit_inp %>% 
  mutate(label = if_else(date_time <end-days(2), 'train', 'test'))
###
...
lightgbm_model_final <- lightgbm_model %>% 
  finalize_model(lightgbm_best_params)


test_res <- predict(lgbm_mod, filter(fit_inp, label=='test')) %>% 
  bind_cols(filter(fit_inp, label=='test')) %>%
  mutate(type = 'backfit')


ggplot()+
  geom_line(data=prod_res, aes(x=datetime, y = .pred), color='red', alpha=0.5)+
  geom_line(data=prod_res, aes(x=datetime, y = total_load))+
  geom_line(data=test_res, aes(x=datetime, y = .pred), color='blue', alpha=0.5)+
  theme_bw()
